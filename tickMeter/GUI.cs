...
namespace tickMeter
{
    ...
    public partial class GUI : Form
    {
        ...
        string[] settings = { "Beállítások", "Settings" };
        string[] rtssOutput = { "RivaTuner kimenet (overlay)", "RivaTuner output (overlay)" };
        string[] logging = { "Logolás Excel fájlba", "Logging into Excel file" };
        string[] ipAddress = { "IP cím megjelenítése (szerver)", "Show IP address (server)" };
        string[] pingRegion = { "Ping és régió megjelenítése", "Show ping and region" };
        string[] networkTraffic = { "Hálózati forgalom megjelenítése", "Show network traffic" };
        string[] showGraph = { "Grafikon megjelenítése", "Show graph" };
        string[] networkCard = { "Hálókártya:", "Network card:" };
        string[] language = { "Nyelv:", "Language:" };
        string[] translation = { "Fordítás: xHeavenHF", "Translation: xHeavenHF" };
            ...
            for (int i = 0; i != AdaptersList.Count; ++i)
            {
                ...
                if (Adapter.Description != null)
                {
                    try // crash ghetto fix
                    {
                        ...
                        adapters_list.Items.Add(match.Value + " - " + Adapter.Description.Replace("Network adapter ", "").Replace("'Microsoft' ", "").Replace("on local host", "").Replace("'", ""));
                    }
                    catch { }
                }
                else
        ...
        {
            int index = comboBoxLanguage.SelectedIndex;

            settings_lbl.Text = settings[index];
            settings_rtss_output.Text = rtssOutput[index];
            settings_log_checkobx.Text = logging[index];
            settings_ip_checkbox.Text = ipAddress[index];
            settings_ping_checkbox.Text = pingRegion[index];
            settings_traffic_checkbox.Text = networkTraffic[index];
            settings_chart_checkbox.Text = showGraph[index];
            network_connection_lbl.Text = networkCard[index];
            labelLanguage.Text = language[index];
            labelTranslation.Text = translation[index];
        }
    }
}
